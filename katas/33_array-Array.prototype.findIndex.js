...
describe('`Array.prototype.findIndex` makes finding items in arrays easier', () => {
  ...
  it('returns the first position it was found at', function() {
    const foundAt = [0, 1, 1, 1].findIndex(item => item == 1); //const foundAt = [0, 1, 1, 1].findIndex(item => item = 1);

    assert.equal(foundAt, 1);
  });

  it('returns `-1` when nothing was found', function() {
    const foundAt = [1, 1, 1].findIndex(item => item > 1); //const foundAt = [1, 2, 3].findIndex(item => item > 1);
  ...
  it('the findIndex callback gets the item, index and array as arguments', function() {
    ...
    function theSecondThree(element, index, arr) { //function theSecondThree(index, arr) {
  ...
  it('combined with destructuring complex compares become short', function() {
    ...
    const foundAt = [bob, alice].findIndex(({name}) => name.length > 3); //const foundAt = [bob, alice].findIndex(({name:{length:l}}) => length > 3);
   /*otra forma:  const foundAt = [bob, alice].findIndex(({name:{length:l}}) => l > 3);*/
    assert.equal(foundAt, 1);
  });
});
