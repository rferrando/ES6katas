...
class ConsumableUsers {
  constructor() {
    ...
    this.nousers = false; /*añadido*/
  }
  get nextUser() {
    if (this.users.length > 0) {
      return `user: ${this.users.shift()}`;
    }
    else {this.nousers = true}  /*añadido*/
    return void 0;
  }
  get anyLeft() {
    return !this.nousers; //return this.users.length > 0;
...
describe('Iterator usages', () => {
  ...
  beforeEach(function(){
    ...
    function iteratorFunction() {
      return {
        next: function() {
          return {value: consumableUsers.nextUser, done: !consumableUsers.anyLeft}//  return {value: consumableUsers.nextUser, done: consumableUsers.anyLeft}
        }
      }
    }
    usersIterable = {[Symbol.iterator]: iteratorFunction};//usersIterable = {};
  });
  ...
  describe('fill the iterable with content using `ConsumableUsers`', function() {
    ...
    describe('using built-in constructs', function() {
      it('use `Array.from()` to convert an iterable to an array', function() {
        const users = Array.from(usersIterable); //const users = usersIterable;
        assert.deepEqual(users, ['user: Alice', 'user: Bob']);
      });
      it('use for-of to loop over an iterable', function() {
        const users = [];
        for (let user of usersIterable) users.push(user); //for (let user in usersIterable) users.push(user);
        assert.deepEqual(users, ['user: Alice', 'user: Bob']);
      });
      it('use the spread-operator to convert/add iterable to an array', function() {
        const users = ['noname', ...usersIterable]; //const users = ['noname', usersIterable];
        assert.deepEqual(users, ['noname', 'user: Alice', 'user: Bob']);
      });
      it('destructure an iterable like an array', function() {
        const [firstUser, secondUser] = usersIterable; //const {firstUser, secondUser} = usersIterable;
