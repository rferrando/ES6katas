...
describe('`catch()` returns a Promise and deals with rejected cases only', () => {
  ...
  describe('`catch` method basics', () => {
    it('is an instance method', () => {
      const p = Promise.resolve(); //or  const p = new Promise(() => {}); //const p = Promise;
    ...
    it('returns a new promise', () => {
      const whatToReturn = () => Promise.resolve(); //const whatToReturn = () => Promise.reject();
    ...
    it('converts it`s return value into a promise', () => {
      ...
      const p1 = p.catch(() => 'promise?'); //const p1 = p.catch(() => void 0);

      return p1.then(result => assert.equal('promise?', result));
    });

    it('the first parameter is the rejection reason', () => {
      const p = Promise.reject('oops'); //const p = Promise.reject('rejection');
  ...
  describe('multiple `catch`es', () => {
    it('only the first `catch` is called', () => {
      ...
      const p1 = p
          .catch(reason => `${reason} AND 2`) //catch(reason => void 0)
    ...
    it('if a `catch` throws, the next `catch` catches it', () => {
      ...
      const p1 = p
          ...
          .catch(err => `${err.message}`) //.catch(err => `${err} but NOT THIS`)
        ;
