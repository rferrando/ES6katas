...
describe('`[].entries()` returns an iterator object with all entries', function() {
  ...
  it('returns key+value for each element', function() {
    const arr = ['a', 'b', 'c'];
    const entriesAsArray = Array.from(arr.entries()); //  const entriesAsArray = Array.from(arr.___());
  ...
  it('empty elements contain the value `undefined`', function() {
    const arr = ['one'];
    arr[2] = 'three';
    const secondValue = Array.from(arr.entries())[1]; //const secondValue = arr.entries();
  ...
  describe('returns an iterable', function() {
    ...
    it('has `next()` to iterate', function() {
      const arr = ['one'];
      const value = arr.entries().next().value; //  const value = arr;
