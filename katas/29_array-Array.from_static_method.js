...
describe('`Array.from` converts an array-like object or list into an Array', () => {
  ...
  it('call `Array.from` with an array-like object', function() {
    const arr = Array.from(arrayLike); //const arr = arrayLike;
  ...
  it('a DOM node`s classList object can be converted', function() {
    ...
    const classList =  Array.from(domNode.classList); //const classList =  domNode.classList;

    assert.equal(''+classList, ''+['some', 'other']);
  });

  it('convert a NodeList to an Array and `filter()` works on it', function() {
    const nodeList = Array.from(document.createElement('span')); //const nodeList = document.createElement('span');
  ...
  describe('custom conversion using a map function as second param', () => {
    it('we can modify the value before putting it in the array', function() {
      const arr = Array.from(arrayLike, (value) => value.toUpperCase()); //const arr = Array.from(arrayLike, (value) => value);

      assert.deepEqual(arr, ['ONE', 'TWO']);
    });

    it('and we also get the object`s key as second parameter', function() {
      const arr = Array.from(arrayLike, (value, key) => `${key}=${value}`);   //const arr = Array.from(arrayLike, (value) => `${key}=${value}`);
